...
import { FirebaseError as u, base64 as _, DecodeBase64StringError as l, getDefaultEmulatorHostnameAndPort as c, createMockUserToken as h, getModularInstance as d, deepEqual as f } from "@firebase/util";
...
function __PRIVATE_decodeBase64(t) {
    try {
        ...
        throw t instanceof l ? new __PRIVATE_Base64DecodeError("Invalid base64 string: " + t) : t;
    }
}
...
class __PRIVATE_DatastoreImpl extends class Datastore {} {
    ...
    tt() {
        if (this.X) throw new FirestoreError(F, "The client has already been terminated.");
    }
...
/** A "context" object passed around while parsing user data. */ class __PRIVATE_ParseContextImpl {
    ...
    ut(t) {
        ...
        if (__PRIVATE_isWrite(this.nt) && z.test(t)) throw this.ct('Document fields cannot begin and end with "__"');
    }
}
...
/** Parse update data from a list of field/value arguments. */ function __PRIVATE_parseUpdateVarargs(t, e, r, n, i, s) {
    ...
    const c = new FieldMask(_);
    return new ParsedUpdateData(l, c, o.fieldTransforms);
}
 ...
 */ function __PRIVATE_validateNewFieldFilter(t, e) {
    if (e.isInequality()) {
        ...
        if (null !== r && !r.isEqual(n)) throw new FirestoreError(R, `Invalid query. All where filters with an inequality (<, <=, !=, not-in, >, or >=) must be on the same field. But you have inequality filters on '${r.toString()}' and '${n.toString()}'`);
...
function __PRIVATE_validateOrderByAndInequalityMatch(t, e, r) {
    if (!r.isEqual(e)) throw new FirestoreError(R, `Invalid query. You have a where filter with an inequality (<, <=, !=, not-in, >, or >=) on field '${e.toString()}' and so you must also use '${e.toString()}' as your first argument to orderBy(), but your first orderBy() is on field '${r.toString()}' instead.`);
}
